                ...
                <div class="row">
                    ...
                    <div class="col-md-6 form-group d-none" id="bonus-file-group">
                        ...
                        @if ($isEdit && $bonusAplikasi->file_path)
                            <a href="{{ route('admin.bonus_aplikasi.download', $bonusAplikasi->id) }}"
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        @if ($isEdit && $bonusAplikasi->thumbnail_path)
                            <img src="{{ asset($bonusAplikasi->thumbnail_path) }}"
                                alt="Thumbnail {{ $bonusAplikasi->nama }}" class="img-thumbnail d-block mt-2"
                    ...
                    <tbody>
                        @foreach ($bonusAplikasiList as $item)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="badge {{ $item->status === 'upcoming' ? 'badge-warning' : 'badge-success' }}">
                                        {{ $item->status === 'upcoming' ? 'Upcoming' : 'Non upcoming' }}
                                    </span>
                                </td>
                                <td>
                                    @if ($item->tipe_sumber === 'url')
                                        <a href="{{ $item->url }}" target="_blank" rel="noopener noreferrer">Link
                                            URL</a>
                                    @else
                                ...
                                <td>
                                    <a href="{{ route('admin.bonus_aplikasi.edit', $item->id) }}"
                                    ...
                                    <form action="{{ route('admin.bonus_aplikasi.destroy', $item->id) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-sm btn-danger" data-delete-bonus
                                            title="Hapus">
