    ...
    <div class="container pt-5">
        <div class="row justify-content-center">
            <div class="col-xl-10 col-lg-11">
                ...
                <div class="mb-4">
                    <a href="{{ route('siswa.materi.belajar', [$materi->id]) }}"
                        class="text-muted text-decoration-none font-weight-bold small">
                ...
                <div class="card border-0 shadow-lg overflow-hidden" style="border-radius: 24px; background: #ffffff;">
                    <div class="row no-gutters">
                        ...
                        <div class="col-md-6 p-4 p-lg-5 text-white d-flex flex-column justify-content-between"
                            style="background: linear-gradient(135deg, #1E1B4B 0%, #312E81 100%);">
                            <div>
                                <span class="badge badge-pill px-3 py-2 mb-4"
                                    style="background: rgba(255, 255, 255, 0.15); color: #F3F4F6;">
                                    <i class="fas fa-wallet mr-2 text-warning"></i> Checkout Pelatihan
                                </span>
                                <h4 class="text-muted small uppercase font-weight-bold mb-1" style="letter-spacing: 1px;">
                                    MODUL PROGRAM</h4>
                                <h2 class="font-weight-bold mb-3" style="line-height: 1.3; font-size: 1.75rem;">
                                    {{ $materi->nama }}</h2>
                            ...
                            <div class="mt-5 pt-4 border-top" style="border-color: rgba(255,255,255,0.1) !important;">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="bg-warning text-dark p-3 rounded-circle d-none d-sm-flex align-items-center justify-content-center"
                        ...
                        <div class="col-md-6 p-4 p-lg-5 bg-white d-flex flex-column justify-content-center">
                            ...
                            <p class="text-muted small mb-4">Mohon periksa data diri Anda sebelum melanjutkan pemrosesan
                                modul.</p>
                            ...
                            @if ($statusBeasiswaSiswa == 1)
                                ...
                                <div class="p-3 rounded mb-4 text-center {{ $saldoCukup ? 'bg-light-success text-success border border-success' : 'bg-light-danger text-danger border border-danger' }}"
                                    style="border-radius: 12px; background-color: {{ $saldoCukup ? '#f0fdf4' : '#fef2f2' }};">
                                    <span class="small d-block text-muted mb-1">SALDO BEASISWA ANDA</span>
                                    <h4 class="font-weight-bold mb-1">Rp
                                        {{ number_format($siswaProfile->saldo ?? 0, 0, ',', '.') }}</h4>

                                    @if (!$saldoCukup)
                                        <small class="font-weight-bold d-block mt-2">
                                            <i class="fas fa-exclamation-triangle mr-1"></i> Saldo Beasiswa Anda Tidak
                                            Mencukupi. Silakan lakukan pembayaran online di bawah ini.
                                        </small>
                                    @else
                                        <small class="d-block text-muted mt-1">Saldo mencukupi untuk melakukan pemotongan
                                            otomatis.</small>
                                    @endif
                                </div>
                                ...
                                @if ($saldoCukup)
                                    ...
                                    <form action="{{ route('siswa.materi.bayar_beasiswa', $materi->id) }}" method="POST">
                                        @csrf
                                        <button type="submit"
                                            class="btn btn-success btn-block btn-lg py-3 font-weight-bold shadow-md"
                                            style="border-radius: 14px; border: none; font-size: 1.05rem;">
                                ...
                                @else
                                    ...
                                    <form action="{{ route('payment.order.material') }}" method="POST">
                                        ...
                                        <input type="hidden" name="nomor_handphone"
                                            value="{{ $siswaProfile->no_telp ?? '08123456789' }}">

                                        <button type="submit"
                                            class="btn btn-primary btn-block btn-lg py-3 font-weight-bold shadow-md"
                                            style="border-radius: 14px; background: #4F46E5; border: none; font-size: 1.05rem;">
                            ...
                            @else
                                <h5 class="font-weight-bold text-dark mb-3" style="font-size: 0.95rem;">Metode Pembelajaran
                                    Yang Didapatkan:</h5>
                                <ul class="list-unstyled text-secondary small mb-4">
                                    <li class="mb-2"><i class="fas fa-check-circle text-success mr-2"></i> Akses Selamanya
                                        Video Pelajaran</li>
                                    <li class="mb-2"><i class="fas fa-check-circle text-success mr-2"></i> Unduh Modul
                                        Pendukung Materi (PDF)</li>
                                    <li class="mb-2"><i class="fas fa-check-circle text-success mr-2"></i> Sertifikat
                                        Kelulusan Resmi Elektronik</li>
                                </ul>
