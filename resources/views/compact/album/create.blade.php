...
<div class="container-fluid px-4 my-4">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
        ...
        <div class="d-flex align-items-center gap-2">
            <button type="button" class="btn btn-primary rounded-pill px-4 py-2 fw-semibold shadow-sm" onclick="openUploadModal()">
            ...
            <div class="row g-2 align-items-center">
                ...
                <div class="col-md-4 col-12">
                    <div class="input-group">
                        <input type="text" id="filterSearch" class="form-control bg-light border-start-0 rounded-pill" placeholder="Cari nama foto..." oninput="debounceFilter()">
                    </div>
                </div>

                <!-- Filter Ukuran File -->
                <div class="col-md-3 col-6">
                    <select id="filterSize" class="form-select form-control bg-light rounded-pill" onchange="loadAlbumPhotos()">
                ...
                <div class="col-md-2 col-6">
                    <input type="date" id="filterDateFrom" class="form-control bg-light rounded-pill" title="Dari Tanggal" onchange="loadAlbumPhotos()">
                </div>
                <div class="col-md-2 col-6">
                    <input type="date" id="filterDateTo" class="form-control bg-light rounded-pill" title="Sampai Tanggal" onchange="loadAlbumPhotos()">
                </div>

                <!-- Reset Filter Button -->
                <div class="col-md-1 col-6 d-grid">
                    <button type="button" class="btn btn-outline-secondary rounded-pill" onclick="resetFilters()" title="Reset Filter">
                        Reset
                    </button>
                </div>
            </div>
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 p-3 bg-light rounded-4 gap-2 border">
        <div class="form-check mb-0">
            <input class="form-check-input photo-checkbox-custom" type="checkbox" id="selectAll" onchange="toggleSelectAll(this)">
            <label class="form-check-label text-dark fw-medium small ms-2 ml-4 align-middle" for="selectAll"> Pilih Semua</label>
        </div>

        <div class="d-flex align-items-center gap-3">
            <div class="btn-group btn-group-sm" role="group" aria-label="View Mode">
                <button type="button" class="btn btn-outline-secondary active" id="btnModeGrid" onclick="switchViewMode('grid')" title="Mode Masonry (Pinterest)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-grid-fill" viewBox="0 0 16 16">
                        <path d="M1 2.5A1.5 1.5 0 0 1 2.5 1h3A1.5 1.5 0 0 1 7 2.5v3A1.5 1.5 0 0 1 5.5 7h-3A1.5 1.5 0 0 1 1 5.5v-3zm8 0A1.5 1.5 0 0 1 10.5 1h3A1.5 1.5 0 0 1 15 2.5v3A1.5 1.5 0 0 1 13.5 7h-3A1.5 1.5 0 0 1 9 5.5v-3zm-8 8A1.5 1.5 0 0 1 2.5 9h3A1.5 1.5 0 0 1 7 10.5v3A1.5 1.5 0 0 1 5.5 15h-3A1.5 1.5 0 0 1 1 13.5v-3zm8 0A1.5 1.5 0 0 1 10.5 9h3a1.5 1.5 0 0 1 1.5 1.5v3a1.5 1.5 0 0 1-1.5 1.5h-3A1.5 1.5 0 0 1 9 13.5v-3z"/>
                    </svg>
                </button>
                <button type="button" class="btn btn-outline-secondary" id="btnModeList" onclick="switchViewMode('list')" title="Mode Detail List">
            ...
            <span id="selectedCount" class="badge bg-secondary-subtle text-secondary rounded-pill px-3 py-2">0 terpilih</span>
            <button type="button" class="btn btn-danger btn-sm rounded-pill px-3" id="btnBatchDelete" onclick="deleteSelectedPhotos()" disabled>
                Hapus Terpilih
            </button>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow rounded-4">
            ...
            <div class="modal-body p-4">
                <form id="albumForm" action="{{ route('album.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mb-3 d-none" id="titleGroup">
                        ...
                        <input type="text" name="title" id="photo_title" class="form-control rounded-3" placeholder="Masukkan judul foto">
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-secondary" id="labelFileInput">Pilih / Drag Foto</label>
                        <div class="upload-zone text-center p-4" id="dropZone" onclick="document.getElementById('photo_image').click()">
                            <div class="text-primary mb-2">
                                <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" fill="currentColor" class="bi bi-images" viewBox="0 0 16 16">
                                    <path d="M4.5 3a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h7a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5h-7zM3 3.5A1.5 1.5 0 0 1 4.5 2h7A1.5 1.5 0 0 1 13 3.5v9a1.5 1.5 0 0 1-1.5 1.5h-7A1.5 1.5 0 0 1 3 12.5v-9z"/>
                                    <path d="M10.5 5a1 1 0 1 0 0-2 1 1 0 0 0 0 2zm-7 7 2.146-2.146a.5.5 0 0 1 .708 0l.646.647.646-.647a.5.5 0 0 1 .708 0L10.5 12H3.5z"/>
                                </svg>
                            </div>
                            <p class="small fw-semibold text-dark mb-1" id="dropZoneText">Klik atau seret foto ke sini</p>
                            <span class="text-muted d-block" style="font-size: 11px;">Bisa pilih multiple file (JPG, PNG, WEBP, Max 2MB/foto)</span>
                        </div>
                        <input type="file" name="images[]" id="photo_image" class="d-none" accept="image/*" multiple onchange="handleFileSelect(this)">
                    </div>
                    ...
                    <div class="mb-3 d-none" id="editPreviewContainer">
                        ...
                        <img id="editPreviewImg" src="" class="img-thumbnail rounded-3 shadow-sm" style="max-height: 140px;">
                    </div>

                    <div class="d-grid gap-2 mt-4">
                        <button type="submit" class="btn btn-primary rounded-pill py-2 fw-semibold" id="btnSubmit">
                            Unggah Sekarang
                        </button>
                    </div>
                </form>
...
<script>
    ...
    // Debounce agar tidak melakukan request berlebihan saat mengetik
    ...
    // Fetch Data dengan Query Parameter Filter
        ...
        if (photosData.length === 0) {
            container.innerHTML = `
                <div class="text-center text-muted py-5">
                    <p class="mb-0 fs-5">Tidak ada foto yang ditemukan.</p>
                    ...
                    <div class="pin-overlay">
                        ...
                        <div>
                            ...
                            <div class="d-flex gap-2">
                                ...
                                <button type="button" class="btn btn-sm btn-danger rounded-pill px-3 py-1 fw-medium" onclick="deleteSinglePhoto(${photo.id})">Hapus</button>
                            </div>
                        </div>
                    </div>
                </div>`;
                ...
                return `
                    <tr id="photo-row-${photo.id}">
                        ...
                        <td class="text-end">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-danger rounded-pill px-3" onclick="deleteSinglePhoto(${photo.id})">Hapus</button>
                        </td>
                    </tr>`;
            }).join('');
    ...
    function handleFileSelect(input) {
        ...
        if (input.files.length > 0) {
            ...
            Array.from(input.files).forEach(file => {
                ...
                li.className = 'list-group-item d-flex justify-content-between align-items-center px-0 py-1 border-0 bg-transparent';
                li.innerHTML = `<span class="text-truncate" style="max-width: 200px;">${file.name}</span> <span class="badge bg-light text-dark">${(file.size / 1024 / 1024).toFixed(2)} MB</span>`;
    ...
    async function deleteSelectedPhotos() {
        ...
        if (!confirm(`Hapus ${selectedPhotoIds.size} foto terpilih?`)) return;
        await executeDelete(Array.from(selectedPhotoIds));
    }

    async function executeDelete(ids) {
        try {
            const res = await fetch("{{ route('album.destroyBatch') }}", {
                method: 'DELETE',
                headers: {
