    ...
    <div class="course-list-page">
        <section class="course-list-hero" aria-labelledby="course-list-title">
            <div class="course-list-hero__content">
                ...
                <h1 class="course-list-hero__title" id="course-list-title">Pilih kelas pembelajaran yang sesuai target karier
                    Anda</h1>
                <p class="course-list-hero__description">
                    Temukan kelas, event, dan pelatihan perbankan yang dirancang untuk meningkatkan kompetensi secara
                    praktis dan terukur.
                </p>
            </div>
        </section>
        ...
        <div class="course-list-layout">
            <aside class="course-filter-card" aria-label="Filter kelas pembelajaran">
                ...
                <p class="course-filter-card__subtitle">Pilih kategori, instruktur, level, jenis, mode, dan tipe IHT kelas.
                </p>
                ...
                <form class="course-filter-form" method="GET" action="{{ url('/event-kelas') }}">
                    <div class="course-filter-field">
                        ...
                        <input type="search" id="course-search" name="q" class="course-filter-control"
                            value="{{ $filters['q'] }}" placeholder="Cari kelas anda">
                    </div>
                    ...
                    <div class="course-filter-field">
                        ...
                        <select id="course-category" name="category" class="course-filter-control">
                            ...
                            @foreach ($filterOptions['category'] as $value => $label)
                                <option value="{{ $value }}"
                                    {{ in_array((string) $value, $filters['category'], true) ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="course-filter-field">
                        ...
                        <select id="course-instructor" name="instructor" class="course-filter-control">
                            ...
                            @foreach ($filterOptions['instructor'] as $value => $label)
                                <option value="{{ $value }}"
                                    {{ in_array((string) $value, $filters['instructor'], true) ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="course-filter-field">
                        ...
                        <select id="course-level" name="level" class="course-filter-control">
                            ...
                            @foreach ($levelOptions as $value => $label)
                                <option value="{{ $value }}"
                                    {{ (string) $filters['level'] === (string) $value ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="course-filter-field">
                        ...
                        <select id="course-jenis" name="jenis" class="course-filter-control">
                            ...
                            @foreach ($filterOptions['jenis'] as $value => $label)
                                <option value="{{ $value }}"
                                    {{ in_array((string) $value, $filters['jenis'], true) ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="course-filter-field">
                        ...
                        <select id="course-kategori" name="kategori" class="course-filter-control">
                            ...
                            @foreach ($filterOptions['kategori'] as $value => $label)
                                <option value="{{ $value }}"
                                    {{ in_array((string) $value, $filters['kategori'], true) ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="course-filter-field">
                        ...
                        <select id="course-iht" name="iht" class="course-filter-control">
                            ...
                            @foreach ($filterOptions['iht'] as $value => $label)
                                <option value="{{ $value }}"
                                    {{ in_array((string) $value, $filters['iht'], true) ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <section class="course-list-content" aria-labelledby="course-grid-title">
                ...
                @if ($classes->count() > 0)
                    <div class="course-grid" id="courseGrid">
                        @include('membernonkeanggotaan.components.ui.course-card-items', [
                            'classes' => $classes,
                        ])
                    </div>

                    @if (method_exists($classes, 'hasPages') && $classes->hasPages())
                        <div class="course-infinite-scroll" id="courseInfiniteScroll"
                            data-next-url="{{ $classes->nextPageUrl() }}">
                            <button type="button" class="course-load-more-button" id="courseLoadMoreButton">Muat kelas
                                lainnya</button>
                ...
                @else
                    <div class="course-empty-state">
                        ...
                        <p>Ubah filter kategori, instruktur, level, jenis, mode, atau IHT untuk melihat kelas pembelajaran
