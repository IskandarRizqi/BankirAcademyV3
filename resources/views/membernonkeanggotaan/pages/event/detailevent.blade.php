...
@section('content')
    @php
        ...
        $registrationEnd = $endDate
            ...
            : ($registrationStart
                ? $registrationStart->copy()->endOfDay()
                : null);
        ...
        if ($registrationEnd && $today->greaterThan($registrationEnd)) {
            ...
            $registrationStart &&
            $registrationEnd &&
            $today->betweenIncluded($registrationStart, $registrationEnd)
        ) {
        ...
        if ($isPriceComingSoon) {
            $registrationAlert = [
                ...
                'text' =>
                    'Investasi kelas ini belum tersedia. Silakan cek kembali nanti atau hubungi tim Bankir Academy.',
            ];
        }
    @endphp
                ...
                <div class="event-hero-visual">
                    <div class="event-cover-card">
                        <img src="{{ $image }}" alt="{{ $title }}"
                            onerror="this.src='{{ asset('cbtemplate/assets/img/90x90.jpg') }}'">
                    ...
                    <div class="event-price-strip">
                        <div>
                            ...
                            <span
                                class="event-price-value {{ $isPriceComingSoon ? 'event-price-value--coming-soon' : '' }}">{{ $priceLabel }}</span>
                            @if (!$isPriceComingSoon && $promoPrice > 0 && $price > $finalPrice)
                                <span class="event-price-original">Rp {{ number_format($price, 0, ',', '.') }}</span>
                            @endif
                        </div>
                        @if ($isIhtOrderable)
                            <form action="{{ route('membernonanggota.event.order-iht', data_get($class, 'unique_id')) }}"
                                method="POST" target="_blank" rel="noopener" class="m-0 js-iht-order-form">
                                @csrf
        ...
        <div class="event-body-layout">
                ...
                <section class="event-panel" aria-labelledby="event-about-title">
                    <div class="event-panel__body">
                        ...
                        <div class="event-description">
                            ...
                            @else
                                <p>Deskripsi kelas belum tersedia. Silakan hubungi tim Bankir Academy untuk informasi lebih
                                    lanjut.</p>
                            @endif
                        </div>
                    </div>
                            ...
                            <div class="event-agenda-list">
                                @foreach ($events as $event)
                                    <div class="event-agenda-item">
                                        <p class="event-agenda-text">
                                            {{ data_get($event, 'description') ?: 'Sesi pembelajaran kelas' }}</p>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    @if (!$isIhtOrderable)
                        <section class="event-panel" aria-labelledby="event-instructor-title">
                            <div class="event-panel__body">
                                ...
                                <h2 class="event-section-title" id="event-instructor-title">Dibimbing oleh praktisi
                                    berpengalaman</h2>
                                <div class="event-instructor-grid">
                                    @foreach ($instructors as $instructor)
                                        @php
                                            ...
                                            $picture = $pictureUrl
                                                ? asset('Image/' . $pictureUrl)
                                                : data_get($instructor, 'picture');
                                            $picture = $picture ?: asset('cbtemplate/assets/img/90x90.jpg');
                                        @endphp
                                        <div class="event-instructor-card">
                                            <img src="{{ $picture }}"
                                                alt="{{ data_get($instructor, 'name', 'Instruktur') }}"
                                                class="event-instructor-avatar"
                                                onerror="this.src='{{ asset('cbtemplate/assets/img/90x90.jpg') }}'">
                                            <div>
                                                <h3 class="event-instructor-name">
                                                    {{ data_get($instructor, 'name', 'Instruktur Bankir Academy') }}</h3>
                                                <p class="event-instructor-title">
                                                    {{ data_get($instructor, 'title') ?: 'Praktisi dan pengajar Bankir Academy' }}
                                                </p>
                                            </div>
                                        </div>
                                    @endforeach
            ...
            <aside class="event-side-stack" aria-label="Ringkasan kelas">
                <section class="event-register-card">
                    ...
                    <span
                        class="event-register-card__price {{ $isPriceComingSoon ? 'event-register-card__price--coming-soon' : '' }}">{{ $priceLabel }}</span>
                    @if (!$isPriceComingSoon && $promoPrice > 0 && $price > $finalPrice)
                    ...
                    <div class="event-panel__body">
                        ...
                        <div class="event-highlight-grid">
                            ...
                            <div class="event-highlight-card">
                                ...
                                <span
                                    class="event-highlight-card__value">{{ $participantLimit ? $participantLimit . ' peserta' : 'Terbatas' }}</span>
                            </div>
                        </div>
                    </div>
                </section>
                        ...
                        <div class="event-panel__body">
                            ...
                            <div class="event-related-list">
                                @foreach ($relatedClasses->where('status', 1) as $related)
                                    <a href="{{ url('/detail-event/' . $related->unique_id . '/' . \Illuminate\Support\Str::slug($related->title)) }}"
                                        class="event-related-link">
                                        {{ $related->title }}
                                    </a>
                                @endforeach
                            </div>
    ...
    @include('membernonkeanggotaan.components.ui.event-registration-modal', [
        'class' => $class,
        'modalId' => 'eventRegistrationModal',
    ])
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.js-event-order-button').forEach(function(button) {
                button.addEventListener('click', function(event) {
                    ...
                    const alertText = button.dataset.alertText ||
                        'Kelas ini belum dapat diorder saat ini.';
