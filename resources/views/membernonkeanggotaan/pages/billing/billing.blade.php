    ...
    <section class="billing-history-section" aria-labelledby="billing-history-title">
        ...
        @if ($paymentHistories->count() > 0)
            <div class="billing-history-list" id="billingHistoryList">
                @include('membernonkeanggotaan.components.ui.billing-history-items', [
                    'paymentHistories' => $paymentHistories,
                ])
            </div>

            @if (method_exists($paymentHistories, 'hasPages') && $paymentHistories->hasPages())
                <div class="billing-history-loader" id="billingHistoryLoader"
                    data-next-url="{{ $paymentHistories->nextPageUrl() }}">
                    <button type="button" class="billing-history-load-more" id="billingHistoryLoadMoreButton">Muat histori
                        lainnya</button>
