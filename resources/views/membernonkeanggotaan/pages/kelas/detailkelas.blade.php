...
$courseTimeLabel = $courseTime
? \Carbon\Carbon::parse($courseTime)->format('H:i') . ' WIB'
: 'Menyesuaikan';
...
$contentUnlockLabel = $contentUnlockAt
? $contentUnlockAt->format('d/m/Y H:i') . ' WIB'
: null;
...
$accessLinkIsUrl = filter_var($accessLink, FILTER_VALIDATE_URL)
...
$courseStatus = $endDate && $today->greaterThan(\Carbon\Carbon::parse($endDate)->endOfDay())
? 'Selesai'
: 'Kelas Anda';
    ...
    <section class="member-owned-class-detail__hero" aria-labelledby="owned-class-title">
        <div class="member-owned-class-detail__hero-inner">
            <div class="member-owned-class-detail__hero-content">
                <div>
                    <a href="{{ url('/kelas-event') }}" class="member-owned-class-detail__back-link">&larr; Kembali ke kelas Anda</a>
                    ...
                    <div class="member-owned-class-detail__eyebrow">
                        ...
                        <span class="member-owned-class-detail__pill member-owned-class-detail__pill--{{ $courseStatusClass }}">{{ $courseStatus }}</span>
                        @if($isIht)
            ...
            <div class="member-owned-class-detail__visual">
                <div class="member-owned-class-detail__cover">
                    <img src="{{ $image }}" alt="{{ $title }}" loading="eager" onerror="this.src='{{ asset('assets/img/90x90.jpg') }}'">
                </div>
    ...
    <div class="member-owned-class-detail__body">
        <main class="member-owned-class-detail__content">
            <section class="member-owned-class-detail__panel" aria-labelledby="owned-class-about-title">
                <div class="member-owned-class-detail__panel-body">
                    ...
                    <h2 class="member-owned-class-detail__section-title" id="owned-class-about-title">Ringkasan pembelajaran</h2>
                ...
                <div class="member-owned-class-detail__panel-body">
                    <div class="member-owned-class-detail__content-header">
                        <div>
                            ...
                            <h2 class="member-owned-class-detail__section-title mb-0" id="owned-class-content-title">Materi pembelajaran</h2>
                        </div>
                    ...
                    @if($contentCount === 0)
                    <p class="member-owned-class-detail__content-empty">Belum ada materi pembelajaran yang tersedia untuk kelas ini.</p>
                    @else
                    <div class="member-owned-class-detail__content-grid">
                        ...
                        \App\Models\ClassContentModel::TYPE_DOCUMENT => ['label' => 'PDF', 'icon' => 'fa-file-pdf', 'class' => 'pdf'],
                        \App\Models\ClassContentModel::TYPE_IMAGE => ['label' => 'Gambar', 'icon' => 'fa-image', 'class' => 'image'],
                        \App\Models\ClassContentModel::TYPE_VIDEO => ['label' => 'Video', 'icon' => 'fa-play-circle', 'class' => 'video'],
                        ][$contentType] ?? ['label' => 'Materi', 'icon' => 'fa-file', 'class' => 'file'];
                        $contentPath = trim((string) $content->url);
                        $hasStoredFile = $contentPath !== '' && $contentPath !== '-';
                        $contentUrl = route('membernonanggota.class.content', ['contentId' => $content->id]);
                        $downloadUrl = $contentUrl . '?download=1';
                        $videoUrl = filter_var($contentPath, FILTER_VALIDATE_URL)
                        ...
                        <article class="member-owned-class-detail__content-card">
                            ...
                            @if($contentUnlocked)
                            <a href="{{ $contentUrl }}" target="_blank" rel="noopener" class="member-owned-class-detail__content-preview member-owned-class-detail__content-preview--image" aria-label="Lihat {{ $content->title }}">
                                <img src="{{ $contentUrl }}" alt="{{ $content->title }}" loading="lazy">
                            </a>
                            @else
                                            <button type="button" class="member-owned-class-detail__content-preview member-owned-class-detail__content-preview--image member-owned-class-detail__content-preview-button" data-content-locked data-alert-text="{{ $contentLockedMessage }}" aria-label="Materi {{ $content->title }} terkunci">
                                    ...
                                    @elseif(! $contentUnlocked)
                                        <button type="button" class="member-owned-class-detail__content-preview member-owned-class-detail__content-preview--{{ $typeMeta['class'] }} member-owned-class-detail__content-preview-button" data-content-locked data-alert-text="{{ $contentLockedMessage }}" aria-label="Materi {{ $content->title }} terkunci">
                                            <span class="member-owned-class-detail__content-logo" aria-hidden="true">
                                                <img src="{{ asset('bankir-academy-icon.png') }}" alt="">
                                            </span>
                                            <i class="fas fa-lock" aria-hidden="true"></i>
                                    ...
                                    @else
                                        <div class="member-owned-class-detail__content-preview member-owned-class-detail__content-preview--{{ $typeMeta['class'] }}" aria-hidden="true">
                            ...
                            <div class="member-owned-class-detail__content-body">
                                ...
                                <h3 class="member-owned-class-detail__content-title">{{ $content->title ?: 'Materi kelas' }}</h3>
                                @if($description !== '')
                                <p class="member-owned-class-detail__content-description">{{ $description }}</p>
                                @endif

                                <div class="member-owned-class-detail__content-actions">
                                    @if(! $contentUnlocked)
                                    <button type="button" class="member-owned-class-detail__content-button member-owned-class-detail__content-button--primary" data-content-locked data-alert-text="{{ $contentLockedMessage }}">
                                    ...
                                    <a href="{{ $videoUrl }}" target="_blank" rel="noopener noreferrer" class="member-owned-class-detail__content-button member-owned-class-detail__content-button--primary">
                                        <i class="fas fa-play" aria-hidden="true"></i> Tonton Video
                                    </a>
                                    @elseif(in_array($contentType, [0, \App\Models\ClassContentModel::TYPE_DOCUMENT, \App\Models\ClassContentModel::TYPE_IMAGE], true) && $hasStoredFile)
                                    <a href="{{ $contentUrl }}" target="_blank" rel="noopener" class="member-owned-class-detail__content-button">
                                        <i class="fas fa-eye" aria-hidden="true"></i> Buka
                                    </a>
                                    <a href="{{ $downloadUrl }}" class="member-owned-class-detail__content-button member-owned-class-detail__content-button--primary">
                                        <i class="fas fa-download" aria-hidden="true"></i> Unduh
                                    </a>
                                    @else
                                    <span class="member-owned-class-detail__content-button">Materi belum tersedia</span>
                                    @endif
                                </div>
                            </div>
                        </article>
        ...
        <aside class="member-owned-class-detail__sidebar" aria-label="Status kelas">
            <section class="member-owned-class-detail__access-card">
                ...
                <p class="member-owned-class-detail__access-text">Anda sudah membeli kelas ini. Tidak ada proses pendaftaran atau pembelian ulang yang diperlukan.</p>
            </section>
            ...
            <section class="member-owned-class-detail__meeting-card" aria-labelledby="owned-class-meeting-title">
                ...
                <div class="member-owned-class-detail__meeting-field">
                    <span class="member-owned-class-detail__meeting-label">{{ $isOfflineEvent ? 'Alamat' : 'Passcode Zoom' }}</span>
                    <span class="member-owned-class-detail__meeting-value">{{ $accessValue ?: ($isOfflineEvent ? 'Alamat belum tersedia' : 'Passcode belum tersedia') }}</span>
                </div>

                <div class="member-owned-class-detail__meeting-actions">
                    @if($accessValue)
                    <button type="button" class="member-owned-class-detail__meeting-button member-owned-class-detail__meeting-button--secondary" data-copy-value="{{ $accessValue }}">
                        <i class="fas fa-copy" aria-hidden="true"></i> Salin {{ $isOfflineEvent ? 'Alamat' : 'Passcode' }}
                    </button>
                    @endif

                    @if($accessLinkUrl)
                    <a href="{{ $accessLinkUrl }}" target="_blank" rel="noopener noreferrer" class="member-owned-class-detail__meeting-button">
                        <i class="fas {{ $isOfflineEvent ? 'fa-map-marker-alt' : 'fa-video' }}" aria-hidden="true"></i>
