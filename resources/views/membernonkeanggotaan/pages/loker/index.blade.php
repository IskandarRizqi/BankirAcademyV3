    ...
    <section class="loker-hero" aria-labelledby="loker-page-title">
        <div class="loker-hero__content">
            ...
            <p class="loker-hero__description">
                Jelajahi lowongan aktif dari perusahaan mitra Bankir Academy dan temukan posisi yang sesuai dengan kompetensi Anda.
            </p>
        </div>
    </section>
        ...
        <aside class="loker-filter-card" aria-label="Filter lowongan kerja">
            ...
            <form method="GET" action="{{ route('membernonanggota.loker.index') }}">
                <div class="loker-filter-field">
                    ...
                    <input class="loker-filter-control" type="search" id="loker-search" name="q" value="{{ $filters['q'] }}" placeholder="Contoh: Relationship Manager">
                </div>
                ...
                <div class="loker-filter-field">
                    ...
                    <select class="loker-filter-control" id="loker-type" name="type">
                        ...
                        @foreach($filterOptions['type'] as $value => $label)
                            <option value="{{ $value }}" {{ $filters['type'] === (string) $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="loker-filter-field">
                    ...
                    <select class="loker-filter-control" id="loker-skill" name="skill">
                        ...
                        @foreach($filterOptions['skill'] as $value => $label)
                            <option value="{{ $value }}" {{ $filters['skill'] === (string) $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="loker-filter-field">
                    ...
                    <select class="loker-filter-control loker-filter-select2" id="loker-province" name="provinsi">
                        ...
                        @foreach($provinces as $province)
                            <option value="{{ $province->id }}" {{ $filters['provinsi'] === (string) $province->id ? 'selected' : '' }}>{{ $province->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="loker-filter-field">
                    ...
                    <select class="loker-filter-control loker-filter-select2" id="loker-city" name="kabupaten" {{ $filters['provinsi'] === '' ? 'disabled' : '' }}>
        ...
        <section aria-labelledby="loker-list-title">
            <div class="loker-section-header">
                <div>
                    ...
                    <p>{{ $isMember ? 'Akses lowongan aktif dari perusahaan mitra.' : 'Peluang pilihan untuk memulai perjalanan karier Anda.' }}</p>
                </div>
            ...
            @if($lokers->count() > 0)
                ...
                @if(method_exists($lokers, 'hasPages') && $lokers->hasPages())
                    <div class="loker-infinite-scroll" id="lokerInfiniteScroll" data-next-url="{{ $lokers->nextPageUrl() }}">
                        <button type="button" class="loker-load-more-button" id="lokerLoadMoreButton">Muat lowongan lainnya</button>
    ...
    @if(!$isMember)
        <section class="loker-membership-cta" aria-labelledby="loker-membership-title">
            <div>
                ...
                <p>
                    Non-member dapat melihat maksimal {{ $nonMembershipLimit }} lowongan. Bergabung sebagai member untuk mendapatkan akses lowongan yang lebih lengkap dan manfaat karier lainnya.
                </p>
            </div>
            <div class="loker-membership-cta__actions">
                <button type="button" class="loker-membership-cta__button loker-membership-cta__button--secondary" data-toggle="modal" data-target="#membershipPackageModal">
                    Member perusahaan
                </button>
                <button type="button" class="loker-membership-cta__button" data-toggle="modal" data-target="#membershipIndividualModal">
                    Member perorangan
                </button>
            </div>
        </section>
