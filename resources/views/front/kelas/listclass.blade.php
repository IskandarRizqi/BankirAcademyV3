...
<script>
    ...
    if (response.data.length > 0) {
        response.data.forEach(dt => {
            // LOGIKA UPCOMING
            // Cek apakah ada kata 'upcoming' di title (case-insensitive)
            const isUpcoming = dt.title.toLowerCase().includes('upcoming');
            ...
            // KONDISI TOMBOL DAFTAR vs UPCOMING
            if (isUpcoming) {
                html += '        <button class="btn btn-secondary btn-block" style="background-color:#ccc; border:none; border-radius:8px; padding:10px 0; font-weight:600; font-size:14px; cursor:not-allowed; color:#666;" disabled>UPCOMING</button>';
            } else {
