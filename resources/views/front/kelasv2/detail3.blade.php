        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px;">
            <form action="" method="POST" enctype="multipart/form-data" id="formInvoice">
                ...
                <div class="modal-header bg-light" style="border-top-left-radius: 12px; border-top-right-radius: 12px;">
                    <div>
                        <h4 class="modal-title font-weight-bold text-dark" id="exampleModalLabel">Formulir Invoice & Pembayaran</h4>
                        <small class="text-muted">Silakan lengkapi data peserta dan metode pembayaran Anda</small>
                    </div>
                ...
                <div class="modal-body p-4">
                        ...
                        <div class="col-md-4 mb-3 mb-md-0">
                            <div class="card border-0 bg-light p-3 h-100" style="border-radius: 8px;">
                                ...
                                <div class="input-group my-auto">
                                    <input class="form-control form-control-lg border-right-0" min="1" type="number" id="jml_pesertas" name="jml_peserta" required onchange="qtyjumlahpeserta()" placeholder="">
                        ...
                        <div class="col-md-8">
                            <div class="card border-0 bg-light p-3 h-100" style="border-radius: 8px;">
                                ...
                                <div class="d-flex style-radio-group" style="gap: 12px;">
                                    ...
                                    <label class="flex-fill m-0 position-relative" style="cursor: pointer;">
                                        <input type="radio" name="sertifikat_invoice_radio" value="0" class="card-radio-input d-none">
                                        <div class="card-radio-box text-center p-2 border rounded bg-white">
                    ...
                    <!-- Section Baru: Pilihan Metode Pembayaran -->
                    <div class="row mb-4">
                        <div class="col-md-12">
                            <div class="card border-0 bg-light p-3" style="border-radius: 8px;">
                                <label class="font-weight-600 text-dark mb-2">Metode Pembayaran</label>
                                <div class="d-flex style-radio-group" style="gap: 12px;">
                                    <!-- Opsi Payment Gateway -->
                                    <label class="flex-fill m-0 position-relative" style="cursor: pointer;">
                                        <input type="radio" name="metode_pembayaran" value="gateway" checked class="card-radio-input d-none">
                                        <div class="card-radio-box text-center p-3 border rounded bg-white">
                                            <i class="fa fa-credit-card text-primary fa-lg mb-2"></i>
                                            <span class="font-weight-bold d-block text-dark" style="font-size: 0.95rem;">Payment Gateway</span>
                                            <small class="text-muted">Otomatis (Virtual Account)</small>
                                        </div>
                                    </label>

                                    <!-- Opsi Manual -->
                                    <label class="flex-fill m-0 position-relative" style="cursor: pointer;">
                                        <input type="radio" name="metode_pembayaran" value="manual" class="card-radio-input d-none">
                                        <div class="card-radio-box text-center p-3 border rounded bg-white">
                                            <i class="fa fa-university text-warning fa-lg mb-2"></i>
                                            <span class="font-weight-bold d-block text-dark" style="font-size: 0.95rem;">Transfer Manual</span>
                                            <small class="text-muted">Konfirmasi manual oleh admin setelah transfer</small>
                                        </div>
                                    </label>
                                </div>
                            </div>
...
function cetakInvoiceSertifikat() {
    ...
    // 2. Validasi Input Dinamis Data Peserta
    ...
    // Ambil nilai pilihan dari Radio Button
    let sertifikatInvoice = $('input[name="sertifikat_invoice_radio"]:checked').val();
    let metodePembayaran = $('input[name="metode_pembayaran"]:checked').val();

    // Set value ke hidden input agar ikut terkirim jika form tersubmit
    $('#sertifikat_invoice').val(sertifikatInvoice);
    $('#jmlp').val(jumlahPeserta);
    $('#payment_invoice').val(metodePembayaran);

    // Ambil data array biodata
    ...
    // 3. Pengkondisian Jalur Endpoint URL Pembayaran
    if (metodePembayaran === 'manual') {
        // --- JALUR TRANSFER MANUAL (Menggunakan AJAX ke /order) ---
        $.ajaxSetup({
        ...
        // --- JALUR PAYMENT GATEWAY (Menggunakan Form Submit Standar ke /order/send) ---
        let form = $('#formInvoice');
        form.attr('action', '/order/send');
        form.attr('target', '_blank'); // Buka tab baru untuk proses invoice pihak ketiga
        
        // Buat input array bayangan (karena serialize array native name="nama[]" terkadang butuh penyesuaian di backend)
        // Jika backend /order/send Anda membaca string JSON, aktifkan baris di bawah ini:
        // $('input[name="nama[]"], input[name="email[]"], input[name="nomor_handphone[]"]').attr('disabled', true); 
