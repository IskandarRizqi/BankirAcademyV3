                                    ...
                                    <select name="loker_skill[]" id="loker_skill" class="form-control" multiple>
                                        ...
                                        @if ($lokerskill)
                                            @foreach (json_decode($lokerskill[0]) as $key => $va)
                                                <option value="{{ $va }}"
                                                    @if (old('loker_skill')) {{-- @foreach (old('loker_skill') as $key => $value)
                                        {{$value==$va?'selected':''}}
                                        @endforeach --}} @endif>
                                                    {{ $va }}</option>
                                            @endforeach
                                    ...
                                    <select name="loker_type[]" id="loker_type" class="form-control" multiple>
                                        ...
                                        @if ($lokertype)
                                            @foreach (json_decode($lokertype[0]) as $key => $val)
                                                <option value="{{ $val }}"
                                                    @if (old('loker_type')) @foreach (old('loker_type') as $key => $value)
                                        {{ $value == $val ? 'selected' : '' }}
                                        @endforeach @endif>
                                                    {{ $val }}</option>
                                            @endforeach
    ...
    <script>
                ...
                columns: [{
                    ...
                    },
                    {
                        "render": function(data, type, row, meta) {
                            return '<img src="/image/loker/' + row.e_gambar +
                                '" style="max-width: 100%; max-height: 90px">';
                        }
                    },
                    {
        ...
        function editloker(id) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // Set langsung ID Perusahaan dan refresh tampilan Select2
        ...
        function deleteLoker(id) {
            Swal.fire({
                ...
                icon: 'warning', // Gantilah 'type' menjadi 'icon' pada SweetAlert2
                showCancelButton: true,
                confirmButtonText: 'Delete',
                padding: '2em'
            }).then(function(result) {
                if (result.isConfirmed) { // Menggunakan isConfirmed sebagai pengganti result.value
